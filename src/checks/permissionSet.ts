...
export function checkPermissionSets(
  projectPath: string,
  ...
  const permissionSetFiles = readdirSync(permissionSetsPath)
  ...
  for (const file of permissionSetFiles) {
    ...
    const visibilities =
      parsed.PermissionSet?.applicationVisibilities;

    if (!visibilities) {
      continue;
    }

    const visibilityList = Array.isArray(visibilities)
      ? visibilities
      : [visibilities];
  ...
  for (const appName of applicationNames) {
    if (!visibleApplications.has(appName)) {
      console.error(
        `✗ ${appName}: no PermissionSet grants application visibility`
      );
      hasError = true;
      continue;
    }
    ...
    console.log(
      `✓ ${appName}: application visibility granted by PermissionSet`
    );
  }
